...
function resolveRuntimeTarget(platform: NodeJS.Platform, arch: string): RuntimeTarget | null {
  if (platform === 'darwin' && arch === 'arm64') {
    return { platform, arch, assetPattern: /_darwin_aarch64\.tar\.gz$/ }
  }
  if (platform === 'darwin' && arch === 'x64') {
    return { platform, arch, assetPattern: /_darwin_amd64\.tar\.gz$/ }
  }
  if (platform === 'linux' && arch === 'arm64') {
    return { platform, arch, assetPattern: /_linux_aarch64\.tar\.gz$/ }
  }
  if (platform === 'linux' && arch === 'x64') {
    return { platform, arch, assetPattern: /_linux_amd64\.tar\.gz$/ }
  }
  if (platform === 'win32' && arch === 'arm64') {
    return { platform, arch, assetPattern: /_windows_aarch64\.zip$/ }
  }
  if (platform === 'win32' && arch === 'x64') {
    return { platform, arch, assetPattern: /_windows_amd64\.zip$/ }
  }
  return null
}
